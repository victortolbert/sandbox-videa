<template lang="pug">
  a(
    v-bind:class = '[ isButton ? "btn btn-" + typeColor : "open checkbox " + typeColor, { active: checked, disabled: disabled, readonly: readonly } ]'
    v-bind:is = 'isButton ? "a" : "label"'
    v-on:click = 'toggle'
  )
    input(
      v-bind:name = 'name'
      v-bind:value = 'checked ? trueValue : falseValue'
      v-if = 'name'
      type = 'hidden'
    )
    span.icon.dropdown-toggle(
      v-if = '!isButton'
      v-bind:class = '[ checked ? "btn-" + typeColor : "", { bg: typeColor === "default" } ]')
    span.icon(
      v-if = '!isButton && checked && typeColor === "default"'
    )
    slot
</template>
